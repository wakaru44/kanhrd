...
Every screenshot in this README — the phone shot above included — is generated by `make screenshots` from a fully mocked bridge: no herdr, no operator socket, a fixed 600-pane fixture and a frozen clock. Each shot is taken twice and discarded unless the two are byte-identical, so regenerating them does not churn Git LFS. See [`apps/web/e2e/capture.spec.ts`](apps/web/e2e/capture.spec.ts).
