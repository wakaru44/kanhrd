...
| | |
| --- | --- |
| <img alt="the kanhrd board, cards grouped across the five status columns and one column of the operator's own, with two workspaces in the rail" src="docs/screenshots/desktop_board.png"> | **The board.** Panes from every configured host in one place, grouped by agent status, with host chips and status filters above. |
