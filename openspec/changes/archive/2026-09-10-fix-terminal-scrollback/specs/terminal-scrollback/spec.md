...
A **snapshot** is one `pane.output` payload: the pane's complete current
content at the subscribed source, never a delta (see `tier-2-terminal`).
**History** is everything above the viewport — the scrollback. A reader is
**at the tail** when the terminal's viewport shows the last screenful, and
**scrolled up** otherwise. An **append** is a snapshot that begins with the
previous snapshot; a **redraw** is any other snapshot.
