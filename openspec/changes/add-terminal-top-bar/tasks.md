...
- [x] 1.1 `pane-detail.ts` — a `siblings` computed over
      `PanesStore.panesSignal`: same `host`, same `tab.id` as the route
      pane, in store iteration order, current pane included
- [x] 1.2 `pane-detail.ts` — extract the display-name precedence used by
...
- [x] 1.3 `pane-detail.spec.ts` — siblings are derived from the store
...
- [x] 2.0 `apps/web/src/app/shared/icons.ts` — add
...
- [x] 2.1 `pane-detail/card-switcher.ts` — a presentational component
...
- [x] 2.2 `shared/copy.ts` — add `nav.cardSwitcher` = `cards in this
...
- [x] 2.5 `card-switcher.scss` — tokens only; selection is `--fw-semi`
...
- [x] 2.6 `card-switcher.spec.ts` — renders one entry per sibling;
...
- [x] 3.1 `pane-detail.html` — breadcrumb `workspace / tab` between the
      back control and the title, in `--ink-mute`; the back control
      stays the first focusable element
- [x] 3.2 `pane-detail.html` — render `<app-card-switcher>` only when
      the tab holds more than one card
- [x] 3.3 `pane-detail.scss` — header rules for the breadcrumb and the
...
- [x] 3.4 `pane-detail.scss` — below `--breakpoint-mobile`: breadcrumb
...
- [x] 3.7 `pane-detail.spec.ts` — the button is absent in a tab of one,
...
- [x] 4.1 `keyboard.service.ts` — a single named constant enumerating
...
- [x] 4.2 `keyboard.service.ts` — in `handleKeydown`, check that list
      *before* the `isTextInputFocused` early return; skip any chord
      ...
      nor `stopPropagation()`
- [x] 4.3 `keyboard.service.ts` — add the `focus-card-switcher` action
      with the chord binding `prefix + i` and the direct binding
      `Ctrl+Alt+I`, category `Navigation`, so both appear in the help
      overlay
- [x] 4.4 `keyboard.service.ts` — add the `next-sibling-card` action,
      ...
      from the pane
- [x] 4.5 `keyboard.service.ts` — dispatch: `focus-card-switcher`
      ...
      in a tab of one
- [x] 4.6 `keyboard.service.spec.ts` — a focused-input `Ctrl+Alt+K` is
...
- [x] 4.7 `keyboard.service.spec.ts` — `prefix + o` routes to the next
      sibling, wraps from last to first, and is a no-op in a tab of one
- [x] 4.8 `keyboard-help-overlay.spec.ts` — all three bindings are
...
- [ ] 5.3 `pnpm --filter @kanhrd/web build` — **DEFERRED, pre-existing
      ...
      is in this change's scope (`apps/web/src/**` only), so raising the
      budget or trimming the initial chunk is a maintainer / follow-up
      lane's call. `pnpm --filter @kanhrd/web test` (652 passing) and
...
- [ ] 5.6 **DEFERRED to the e2e session** (`apps/web/e2e/**` is owned by a
      ...
      *can* make are made in the committed unit suite instead: the
      breadcrumb collapses to the tab name, the switcher still renders, the
      back control is the first focusable element, and
      `documentElement.scrollWidth <= clientWidth`. What karma cannot prove
      — `pointer: coarse` box sizes — is asserted against the compiled
      stylesheet and left for Playwright to measure for real.
      Playwright `mobile` project (390 × 844): the detail route
...
- [ ] 5.7 **DEFERRED — needs a live herdr fixture, which this lane did not
      ...
      element xterm.js uses — but the end-to-end pass-through to a running
      program is unverified.
      Against a private herdr fixture (never the operator's socket —
