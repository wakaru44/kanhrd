...
## A second browser at a different scrollback depth gets the first one's

**Problem**

`kanhrd.terminal-scrollback` is per-operator and persists per browser, but
the bridge's poll loops are keyed by `(host, pane_id)`. The first
subscriber's `lines` therefore decides the depth of the shared loop, and a
second browser watching the same pane at a different depth silently gets
the first one's snapshots. The setting appears to apply and does not.

**Reproduction/current evidence**

Open the same pane in two browsers, set scrollback to 250 in one and 1000
in the other, and compare the live snapshots: both follow whichever
subscribed first. `add-terminal-scrollback-depth` shipped `lines` on
`pane.read` and `pane.subscribe_output` and forwards it through the poller,
but left entry keying untouched — the limit is recorded in that change's
`design.md` and is repeated here because an archived change is not where a
shipped limitation should be looked up.

ADR-0004 (lines 29-31) is the reason it is keyed that way: subscribers to
one `(host, pane_id)` deliberately share a single poll loop and fan out the
same event, rather than each polling herdr independently.

**Expected behavior**

Each browser sees the depth it asked for, or the product does not offer a
per-browser depth.

**Investigation/fix notes**

Keying entries by `(host, pane_id, source, format, lines)` fixes it and is
the obvious move, but it trades away ADR-0004's loop sharing: two operators
at two depths then run two polls against herdr for one pane. That is a
deliberate cost and wants an ADR-0004 amendment rather than a silent
change. The delta-encoding change that follows
`add-terminal-scrollback-depth` has to settle entry keying anyway — it
needs per-subscriber previous-snapshot state for the same reason — so this
should be resolved there rather than on its own.

**Verification/acceptance criteria**

- Two browsers subscribed to one pane at different depths each receive
  their own depth.
- The number of poll loops per pane, and its dependence on distinct
  subscriber options, is stated in ADR-0004 rather than only in a change's
  design notes.
