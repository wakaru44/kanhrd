...
import { originGuard } from '../http/origin.js';
 ...
 *
 * The `Origin` allowlist is enforced in a `preValidation` hook, which runs
 * on the upgrade request before any socket exists: a rejected handshake
 * never reaches the message loop below, so no dispatch verb is reachable
 * from a page the operator merely happened to visit.
 */
export async function registerWebSocket(
  ...
  app.get(
    '/ws',
    { websocket: true, preValidation: originGuard(origins, origins.requireOrigin) },
    (socket) => {
