...
class FakeHost implements PaneReader {
  ...
  async paneRead(params: { pane_id: string; source?: ReadSource; format?: ReadFormat }): Promise<{
    ...
    return { content: next.content, revision: next.revision, truncated: false, format: "ansi", source: "visible" };
  }
}
...
describe("OutputPoller", () => {
  ...
  it("shares one poll loop across multiple subscribers to the same (host, pane_id)", async () => {
    ...
    const subA = poller.subscribe("local", "p1", undefined, undefined, "conn-1", (e) => eventsA.push(e));
    const subB = poller.subscribe("local", "p1", undefined, undefined, "conn-2", (e) => eventsB.push(e));
