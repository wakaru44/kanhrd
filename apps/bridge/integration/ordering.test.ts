...
describe("C. ordering guarantees", () => {
  ...
  it(
    "C1. burst-send ordering: 20 unawaited pane.send_text calls land in exact send order (no keystroke scramble)",
    async (ctx) => {
      ...
      const sends = markers.map((ch) => client.request("local", "pane.send_text", { pane_id: panePid, text: ch }));
      ...
      const enterRes = await client.request("local", "pane.send_keys", { pane_id: panePid, keys: ["Enter"] });
      ...
      try {
        ...
        for (const marker of markers) {
          ...
          const event = await client.waitForEvent(
            (e) =>
              e.event === "pane.output" &&
              (e.payload as { subscription_id: string; content: string }).subscription_id === subscription_id &&
        ...
        await client.call("local", "pane.unsubscribe_output", { subscription_id }).catch(() => undefined);
      }
    },
    15_000,
  );
