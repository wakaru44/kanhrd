/**
 * Just enough markdown for the viewer's `rendered` view: headings,
 * paragraphs, bullet and numbered lists, fenced code, and inline `code` /
 * **strong**.
 *
 * It renders to DATA, not HTML, and the template walks that data. Nothing
 * here reaches `innerHTML`, so a README written by an agent — which is
 * untrusted text like any other file in the checkout — cannot inject markup
 * into the panel.
 *
 * Deliberately not a markdown implementation. The panel reads code and
 * diffs; `rendered` is a convenience for the handful of `.md` files an
 * agent writes, and anything it does not understand degrades to a
