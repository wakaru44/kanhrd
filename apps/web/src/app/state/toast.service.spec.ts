...
describe('ToastService', () => {
  ...
  it('push() enqueues a toast and dismiss() removes it', () => {
    ...
    expect(service.toasts()[0]).toEqual(
      jasmine.objectContaining({ id, level: 'info', message: 'hello' })
    );
  ...
  it('dedups by an explicit key even when the message changes', () => {
    ...
    service.push({
      level: 'warn',
      message: 'lost sight of ada. retrying.',
      key: 'host:ada',
      persistent: true,
    });
    service.push({
      level: 'warn',
      message: 'lost sight of ada. still retrying.',
      key: 'host:ada',
      persistent: true,
    });
  ...
  it('keeps one notice per host when several drop together', () => {
    for (const host of ['ada', 'grace', 'ada', 'grace', 'ada']) {
      service.push({
        level: 'warn',
        message: `lost sight of ${host}. retrying.`,
        key: `host:${host}`,
        persistent: true,
      });
    }
  ...
  it('turns the shown notice into the error, in place, on failure', () => {
    ...
    expect(service.toasts()[0].persistent)
      .withContext('the error times out; the progress notice did not')
      .toBeFalse();
  });
