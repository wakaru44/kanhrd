...
describe('loadSettings (pure): the stored key is read, never reset', () => {
  ...
  it('fills in only the keys the stored payload is missing', () => {
    ...
    expect(loaded.requestedOutputPollIntervalMs).toBe(
      defaultSettings().requestedOutputPollIntervalMs
    );
  });

  it('keeps a stored poll-interval request alongside density', () => {
    const loaded = loadSettings(
      stored('{"density":"compact","requestedOutputPollIntervalMs":250}')
    );
  ...
  it('preserves a stored swimlane dimension', () => {
    expect(loadSettings(stored('{"swimlaneDimension":"checkout"}')).swimlaneDimension).toBe(
      'checkout'
    );
  });
