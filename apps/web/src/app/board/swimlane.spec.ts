...
function band(key: string, label: string, panes: Partial<Record<AgentStatus, Pane[]>> = {}): SwimlaneBand {
  return {
    key,
    label,
    columns: {
...
describe("swimlane: band headings", () => {
  ...
  it("elides a long checkout path at the HEAD, keeping the distinguishing tail", () => {
    ...
    expect(resolved!.label.endsWith("/aservice")).withContext("the tail tells checkouts apart").toBeTrue();
...
describe("Swimlane component", () => {
  ...
  it("shows the heading it is handed, not the raw band label", () => {
    ...
    fixture.detectChanges();
    expect(el().querySelector(".swimlane-title")?.textContent?.trim()).toBe(COPY.swimlane.ungrouped);
  });
