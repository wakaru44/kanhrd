...
describe('ConfirmModal', () => {
  ...
  function render(inputs: {
    ...
    if (inputs.confirmLabel !== undefined)
      fixture.componentRef.setInput('confirmLabel', inputs.confirmLabel);
    if (inputs.cancelLabel !== undefined)
      fixture.componentRef.setInput('cancelLabel', inputs.cancelLabel);
    if (inputs.previewItems !== undefined)
      fixture.componentRef.setInput('previewItems', inputs.previewItems);
  ...
  it('renders one preview row per entity for a cascading close', () => {
    ...
    expect(el.querySelector('.preview-heading')?.textContent).toContain(
  ...
  it('applies the danger class to the confirm button only when danger is true', () => {
    expect(
      render({ title: COPY.confirm.closeTab, danger: true }).el.querySelector('.btn.danger')
    ).toBeTruthy();
    expect(
      render({ title: COPY.confirm.closeTab, danger: false }).el.querySelector('.btn.danger')
    ).toBeFalsy();
  });
