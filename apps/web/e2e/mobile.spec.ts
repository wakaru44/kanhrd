...
async function cardWithActions(page: Page): Promise<Locator | null> {
  const card = allCards(page)
    .filter({ has: page.locator('.card-actions') })
    .first();
  try {
...
test('[3] every visible card renders the compact variant', async ({ app }) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    expect(
      Math.abs(box!.height - compactHeight),
      `card[${i}] height ${box!.height}`
...
test('[4] card title, host seal and overflow trigger fit and do not overlap', async ({ app }) => {
  ...
  test.skip(
    card === null,
    'no host advertises a pane lifecycle capability — no card renders an overflow trigger'
  );
...
test('[9] header hamburger, theme toggle and settings link are visible and tappable', async ({
  app,
}) => {
...
test('[11] one column fills the strip and no neighbour peeks', async ({ app }) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const outside =
      box!.x + box!.width <= stripBox!.x + 1 || box!.x >= stripBox!.x + stripBox!.width - 1;
    expect(outside, `column[${i}] is partially visible beside the current one`).toBe(true);
  }
});

test('[12][14] one swipe advances exactly one column and never rests between pages', async ({
  app,
}) => {
  await expect(allCards(app).first()).toBeVisible({ timeout: 10_000 });
  ...
  expect(
    Math.abs(after.scrollLeft - after.clientWidth),
    'did not settle one page across'
  ).toBeLessThanOrEqual(1);
...
test("[15][16][17][18] the switcher reaches every status and is the strip's selection", async ({
  app,
}) => {
  await expect(allCards(app).first()).toBeVisible({ timeout: 10_000 });
...
test("[19] the switcher stays put while the current column's cards scroll", async ({ app }) => {
  ...
  expect(
    ...
    'the switcher scrolled away with the cards'
  ).toBeLessThanOrEqual(1);
});
...
test('[20] hiding the current status leaves the strip on a visible column', async ({ app }) => {
  ...
  const currentLabel = (
    await segments.nth(currentIndex).locator('.segment-label').textContent()
  )?.trim();
...
test('[21] hiding every status replaces the pager with the no-matches empty state', async ({
  app,
}) => {
  await expect(allCards(app).first()).toBeVisible({ timeout: 10_000 });
...
test.describe('board — no hosts configured', () => {
  ...
  test('[24] the copy button and operating-guide link are visible and tappable', async ({
    page,
  }) => {
...
test('[32][35] the hamburger opens the drawer as an overlay and the backdrop closes it', async ({
  app,
}) => {
  await expect(allCards(app).first()).toBeVisible({ timeout: 10_000 });
...
test('[33][34] the open drawer traps focus, and Escape closes it and restores focus', async ({
  app,
}) => {
  await expect(allCards(app).first()).toBeVisible({ timeout: 10_000 });
  const hamburger = app.locator('.hamburger');
...
test('[36] crossing back above 900px with the drawer open restores the inline rail', async ({
  app,
}) => {
  await expect(allCards(app).first()).toBeVisible({ timeout: 10_000 });
...
test.describe('toasts at 390px', () => {
          ...
          ws.send(
            JSON.stringify({
              ...
              error: {
                ...
                message: 'forced failure for the mobile toast test',
              },
            })
          );
          return;
  ...
  test('[37] a toast stacks below the header, never over it', async ({ page }) => {
    ...
    test.skip(
      (await cardWithActions(page)) === null,
      'no host advertises paneClose — nothing to fail'
    );
...
test('toggling the local host chip still hides and restores cards at mobile width', async ({
  app,
}) => {
  const cards = allCards(app);
  await expect(cards.first()).toBeVisible({ timeout: 10_000 });
