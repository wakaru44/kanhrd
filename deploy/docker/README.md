...
- **Very long host path.** A socket whose host path runs past ~100 characters can be refused from inside the container even though the host connects fine: `sun_path` is 104 bytes. Keep the socket near `~/.config/herdr`.
